...
"""Support for NetFlow files (Rust implementation via subprocess)"""
...
class NetFlow(CmdParser):
    """NetFlow log generator (Rust implementation via subprocess)"""
    ...
    def __init__(self, fdesc: str | BinaryIO, pcap_filter: str | None = None) -> None:
        ...
        pcap_filter: a PCAP filter to use with nfdump (not supported in Rust version)
        """
        if pcap_filter is not None:
            from ivre.utils import LOGGER
            LOGGER.warning("PCAP filter not supported in Rust NetFlow parser")
    ...
    def _parse_file(self):
        ...
        binary_name = "ivre-netflow-parser"
