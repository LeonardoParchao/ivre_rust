...
"""Support for Iptables log from syslog files (Rust implementation via subprocess)"""
...
class Iptables(Parser):
    """Iptables log generator from a syslog file descriptor (Rust implementation via subprocess)"""

    def __init__(self, fname: str, pcap_filter: str | None = None) -> None:
        """Init Iptables class."""
        if pcap_filter is not None:
            from ivre.utils import LOGGER
            LOGGER.warning("PCAP filter not supported in Rust Iptables parser")
        self.fname = fname
        self._results = None
    ...
    def _parse_file(self):
        ...
        binary_name = "ivre-iptables-parser"
